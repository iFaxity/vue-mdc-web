<template lang="pug">
.mdc-text-field(:class=`cssClasses` @touchstart="click" @mousedown="click" @keydown="click" v-ripple="ripple && box && !fullwidth")
  ...
  label.mdc-text-field__label(v-if="!fullwidth", :class=`cssLabelClasses`)
    slot
  i.material-icons.mdc-text-field__icon(v-if=`icon && trailingIcon`, :tabindex=`clickIcon && "0"`) {{icon}}
